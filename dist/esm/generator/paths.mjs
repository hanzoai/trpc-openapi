...
export const getOpenApiPathsObject = (appRouter, securitySchemeNames, filter) => {
    const pathsObject = {};
    const procedures = Object.assign({}, appRouter._def.procedures);
    forEachOpenApiProcedure(procedures, ({ path: procedurePath, type, procedure, meta }) => {
        ...
        try {
            ...
            if (hasInputsDefined && (hasPathParameters || !hasVoidLikeInput)) {
                ...
                if (acceptsRequestBody(method)) {
                    ...
                    requestData.requestParams = getParameterObjects(inputSchema, isInputRequired, pathParameters, requestHeaders, 'path');
                }
                else {
                    requestData.requestParams = getParameterObjects(inputSchema, isInputRequired, pathParameters, requestHeaders, 'all');
                }
            }
            const responses = getResponsesObject(responseSchema, httpMethod, responseHeaders, protect, hasInputs(inputParser), successDescription, errorResponses);
